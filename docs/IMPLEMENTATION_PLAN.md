...
- live official LoCoMo run passed against `D:\Temp\locomo\data\locomo10.json` with no `--limit` using default `--granularity session`: 10 conversations, 1982 answered evidence questions, Recall@5 = 0.834, Recall@10 = 0.934, elapsed_ms = 10362; Recall@50 is explicitly marked saturated because each conversation's session candidate pool is only 19-32 chunks
...
- live official LoCoMo signature session run after temporal context injection now reports multi-k recall, candidate-pool saturation, and per-category diagnostics: Recall@1 = 0.476, Recall@5 = 0.834, Recall@10 = 0.934 across 1982 answered evidence questions, elapsed_ms = 10362; Recall@50 = 1.000 is a saturated metric because top50 covers all 19-32 session chunks per conversation and must not be read as system perfection
- experimental two-stage session-to-dialog retrieval is implemented for LoCoMo without gold-evidence fallback; the official no-limit dialog run does not improve materially yet: Recall@5 = 0.527 and Recall@50 = 0.781, so two-stage routing alone is not sufficient
...
- experimental query-feature rerank is available behind `--query-feature-rerank [balanced|conservative]`; its entity/temporal/quote/rare-token feature extraction now lives in benchmark-neutral `colmem-core` with non-LoCoMo unit tests, while the LoCoMo adapter only wires the opt-in rerank mode
- benchmark output now includes per-category LoCoMo recall diagnostics, actual candidate-pool distribution (`min/max/median/avg` over retrieved candidates rather than total chunks), saturation flags, and miss-taxonomy counters (`gold_absent_from_candidates`, pre-rerank outside top10, final top10/outside top5, rerank moved-down, and two-stage session misses), which separate candidate-generation failures from rank-order failures without using answers/evidence during retrieval
- `HybridRetriever::index_hits_with_diagnostics` now returns final hits, pre-rerank hits, and actual candidate counts from one retrieval pass, so LoCoMo miss taxonomy no longer reruns lexical/vector candidate generation for every question
...
- Ralph follow-up created `.omx/plans/prd-honest-retrieval-rerank.md` and `.omx/plans/test-spec-honest-retrieval-rerank.md` as local execution artifacts for the miss-taxonomy/core-rerank work.

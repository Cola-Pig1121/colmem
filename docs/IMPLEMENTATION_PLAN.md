...
- benchmark output now includes per-category LoCoMo recall diagnostics plus candidate-pool saturation flags, which show category 1/3 dialog questions remain the weakest areas and prevent top-k values from being mistaken for real perfection when k covers the whole candidate pool

...
- benchmark output now includes per-category LoCoMo recall diagnostics, candidate-pool saturation flags, and miss-taxonomy counters (`gold_absent_from_candidates`, pre-rerank outside top10, final top10/outside top5, rerank moved-down, and two-stage session misses), which separate candidate-generation failures from rank-order failures without using answers/evidence during retrieval
